...
public class SandboxTest {
    ...
    public void should_merge_excel_files() throws Exception {
        ...
        for (int sheetIndex = 0; sheetIndex < workbook1.getNumberOfSheets(); sheetIndex++) {
            workbookTarget.createSheet(String.format("%s-%s", excelSource1.getName(), workbook1.getSheetName(sheetIndex)));
        }
        ...
        for (int sheetIndex = 0; sheetIndex < workbook2.getNumberOfSheets(); sheetIndex++) {
            workbookTarget.createSheet(String.format("%s-%s", excelSource2.getName(), workbook2.getSheetName(sheetIndex)));
        }
